#standardSQL
-- Given a list of blockgroups that approximately capture the municipality of Kansas City, MO:
-- Group them by county and aggregate their ACS 2018 5-year data, geometry, and land/water area.
CREATE OR REPLACE VIEW public.nyt_kcmo_agg_bg_by_county AS
SELECT
    ANY_VALUE(blockgroup.state_fips_code) as state_fips_code,
    blockgroup.county_fips_code,
    NULL AS blockgroup_gnis_code,
    NULL AS aff_geo_code,
    ...
    ST_UNION_AGG(blockgroup.blockgroup_geom) AS blockgroup_geom,
...
GROUP BY blockgroup.county_fips_code;